...
→ 문제: 제목이 질문형. 발표 자료는 답을 전달하는 매체이지, 질문을 던지는 매체가 아니다.
  ...
  질문을 던져놓고 "한계 / 이점"이라는 피상적 라벨로 넘어간다.
  청중은 이 슬라이드를 읽고도 "그래서 왜 필요한데?"라고 다시 묻게 된다.

→ 판별법: 제목을 읽었을 때 "이 슬라이드의 주장이 뭐지?"에 바로 답할 수 있는가?
  "왜 Digital Twin이 필요한가?" → 답 불가. 질문일 뿐.
  "비전 단독 한계를 DT 기하 정보로 돌파" → 답 가능. 주장이 명확.

→ 추가 문제: "왜/무엇/어떻게" 질문은 연속되면 발표 전체가 FAQ처럼 읽힌다.
  "왜 필요한가? → 무엇을 했는가? → 어떻게 평가했는가? → 향후 무엇을 할 것인가?"
  이것은 서사가 아니라 인터뷰 질문지이다.
...
→ 핵심: 제목 자체가 "이 슬라이드의 주장"을 담는다.
  "왜 필요한가?"가 아니라 "X로 Y를 돌파한다"는 명확한 입장 표명.
  불릿도 "한계 / 이점"이 아니라 "어떤 한계 → 왜 구조적인가 → 어떻게 해소되는가"의 논증 구조.
```
