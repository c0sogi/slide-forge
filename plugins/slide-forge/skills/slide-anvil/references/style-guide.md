...
→ 판별법: 소제목을 읽고 "이 소제목이 슬라이드의 결론을 말하고 있는가?"
  ...
  → 소제목은 "주제 이름표"이고, 메시지는 첫 번째 L1 불릿의 역할이다.

✅ GOOD:
  ▌하위 집단 분석 결과                 ← 주제만 이름표로
  - 다중 인스턴스 존재 장면에서 DT 효과 극대화   ← 핵심 주장은 첫 L1 불릿에

  ▌주요 실험 결과                     ← 간결한 주제
  - DT 기반 전 방법이 Vision-only 대비 평균 24.3%p 상회  ← 결론은 불릿에서

  ▌단일 vs 복합 기하 신호 비교         ← 짧은 비교 주제
  - 어떤 기하 정보가 성능에 핵심적인지 분리 검증  ← 목적은 불릿에서 설명

→ 핵심: 소제목 = 주제 이름표 (2-8단어 명사구).
  슬라이드의 "So What?" 메시지 = 첫 번째 L1 불릿.
